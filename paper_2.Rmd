...
letest <- presc_y2 %>%
  left_join(test, by = c("row_name" = "ccg",
                          "month" = "lubridate::month(date)")) %>% 
  drop_na(median_nose)

p1 <- letest %>%
  ggscatterstats(quantity, mean_nose)

p2 <- letest %>%
  ggscatterstats(quantity, median_nose)
...
p1 <- letest %>%
  ggscatterstats(quantity, mean_how)

p2 <- letest %>%
  ggscatterstats(quantity, median_how)
