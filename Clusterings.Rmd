...
## *Kronkvist and Engstrom thresholds:*
...
The average number of submission per user is \~7 (6.997224), the standard deviation of reports is \~31 (31.11953). This gives us:
...
I made a quick bar chart of high vs low engagement users according to this approach (ignoring people with zero contributions who are not in the data).
...
Out of curiosity, I tried using the median instead of the mean, which gives::
...
Then fitting the markov mixture models on the output.
...
Using the elbow plot method to
...
It seems like the most appropriate number of clusters is 4.

```{r}
saveRDS(EM_hid4,
     file = here::here("Clusterings", "yr1_k4" ))
     
 saveRDS(hid_clusters4,
     file =here::here("Clusterings", "yr1_k4_clusters" ))    
```
...
#  inner_join(hidden_seq_df) %>%
#  group_by(user_id) %>%
#  mutate(dayOfStudy = 224 - n():1 + 1) %>% View()
```
